...
4. **Gate inventory** — each gate follows the credential triad: account on page A, password clue on page B, gate on page C. Record which layer's search index carries each keyword and what a classified search hit resolves to (gate or `[Access denied]`, never the document). System containers also record the access matrix: which account opens which page (`data-access`) and which login gate grants it (`data-grant`; references/structure/form-system.md §1).
