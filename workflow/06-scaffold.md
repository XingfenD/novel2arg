...
- the tool files copied from this skill's `assets/tools/` into `tools/` (`config.mjs` — shared conventions every checker imports, the one file a renamed project edits — plus `hash.mjs`, `build-keywords.mjs`, `check-links.mjs`, `check-solvable.mjs`, `check-credentials.mjs`, `check-reachability.mjs`, `vendor-alpine.mjs`, `site-model.mjs`, and `site-graph.mjs`; dependency-free, Node built-ins only; the `fixtures/` directory stays in the skill repo), the renderer template `assets/viewer/graph-viewer.html` copied into `viewer/` (site-graph resolves it at `../viewer/graph-viewer.html`), and empty-but-valid keyword table(s) only when M1 is selected
