...
- `index.html` entry shell (start button; rules placeholders only where the profile lists any — its copy lands in step 7 phase 7) carrying the link back to the skill repo
...
- ending pages when M10 is selected, and `docs/` — created if absent, never overwritten — holding the artifacts from steps 1 to 5 as they land
