...
> The step-2 menu. Select the modules the novel actually needs, then assemble them into
...
|---|---|---|---|---|
| M1 | Search index | hashed keyword → hidden page, exact match with synonym aliases; catalog result titles (R9) | the organization would have a search surface; keyword discovery fits the novel | `references/structure/components.md` §1–§2 |
...
| M10 | Endings | dual moral-choice ending reusing just-read documents; fourth-wall close + author link | the novel carries a dilemma; a single ending is fine otherwise | `workflow/03-gdd.md` |
