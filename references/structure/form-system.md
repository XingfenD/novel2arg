...
- A **login gate** authenticates one account: `data-grant="chen"` + `data-next="apps/mail.html"` on the
  ...
  (references/structure/components.md §2–§3). One login box may serve several roles through `data-grants` (§2).
...
- **A derived account is proved by parts, never by printing it.** `check-solvable.mjs` matches a credential
  ...
  is known (references/structure/components.md §3; references/structure/tooling.md §3 item 6).
...
(references/structure/components.md §3). The credential triad still applies (account clue on page A, password clue
...
- State held only in JS with no attribute stays invisible to the checkers — model it with `data-grant` /
  `data-access` or verify manually (references/structure/tooling.md §3 item 1). Per-tab vs cross-tab session
  storage is likewise invisible to `check-solvable` (it models identities as one global set): verify the
  new-tab unlock by hand (references/structure/tooling.md §3 item 7).
