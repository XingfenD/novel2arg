...
| # | 页面 | 层 | 解锁来源 |
|---|---|---|---|
| 06/36 | pages/surface/news.html | 表层 | 导航栏（index 起第 1 跳） |
| 14/36 | pages/platform/login.html | 中层 | 页脚"员工入口" |
| 21/36 | pages/secret/s21-ledger.html | 暗层 | 搜索"癸巳"命中（暗层索引） |

## 4. Gate inventory (excerpt)

| 门（页面） | 账号线索页 A | 密码线索页 B | 凭据三元组推理链 | 承载关键词的索引层 | 机密命中解析为 |
|---|---|---|---|---|---|
| pages/platform/login.html | 员工名录（初始账号，唯一可印出） | 老帖《老蛇的本命年》2013 | 2013 癸巳 → 陈生于 1977 → 密码 `1977` | surface | [Access denied] → 本门 |
...
| 谜题 | 类型（references/design-playbook.md §2） | 守护内容 | 跳数 |
|---|---|---|---|
...
- Atmosphere pages: `pages/surface/menu-2019.html`（non-progression, declared in entry rules）.
